...
func (a *Application) Snapshot(ctx context.Context, prune bool) (Snapshot, error) {
	...
	status, err := operatorstatus.Read(storage, sessionStore(a.config.SessionDir), a.config.MedicoverBaseURL, time.Now().UTC())
	if err != nil {
		return snapshot, err
	}
	...
	for _, action := range status.RequiredActions {
		label := map[string]string{
			...
			"session_unavailable":          "Sprawdź magazyn sesji",
...
func historyDeliveryRowStatus(status, message string) string {
	if operatorstatus.IsCancelledDelivery(status, message) {
		return "anulowane"
	}
...
func actionTarget(action operatorstatus.Action) int {
